...
const PeopleAction = (props) => {
  ...
  const list_people = async () => {
    const res = await axios.get(
      "https://assign-mentor-backend-j4ui.onrender.com/people"
    );
  ...
  const create_student = async (student) => {
    try {
      const res = await axios.post(
        "https://assign-mentor-backend-j4ui.onrender.com/create_students",
        student
      );
      // console.log(res.data);
  ...
  const create_mentor = async (mentor) => {
    try {
      const res = await axios.post(
        "https://assign-mentor-backend-j4ui.onrender.com/create_mentors",
        mentor
      );
      // console.log(res.data);
  ...
  const student_list_for_selected_mentor = async (mentor) => {
    try {
      const res = await axios.get(
        `https://assign-mentor-backend-j4ui.onrender.com/${mentor}/list_students`
      );
  ...
  const add_students_for_mentor = async (mentor, student_ids) => {
    try {
      const res = await axios.post(
        `https://assign-mentor-backend-j4ui.onrender.com/${mentor._id}/add_student`,
        { student: student_ids }
      );
      dispatch({ type: "show_students_forMentor", payload: res.data });
      // console.log(res.data);
  ...
  const add_mentor_for_student = async (student, mentor) => {
    ...
    try {
      const res = await axios.post(
        `https://assign-mentor-backend-j4ui.onrender.com/${student._id}/add_mentor/${mentor[0]}`
      );
      dispatch({
        type: "mentor_history_forStudent",
        payload: res.data.mentorlist,
      });
